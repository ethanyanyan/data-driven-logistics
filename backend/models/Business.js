...
class Business {
  ...
  static async findAll() {
    ...
    try {
      ...
      return rows.map(row => new Business(row.BusinessID, row.BusinessName, row.Description));
    } catch (error) {
  ...
  static async findByID(BusinessID) {
    ...
    try {
      ...
      if (rows.length > 0) {
        return new Business(rows[0].BusinessID, rows[0].BusinessName, rows[0].Description);
      } else {
        return null;
      }
    } catch (error) {
  ...
  async delete() {
    ...
    try {
      ...
      const locations = await connection.query("SELECT * FROM Locations WHERE BusinessID = ?", [this.BusinessID]);
      for (const location of locations[0]) {
        const locationInstance = new Location(location.LocationID, location.BusinessID, location.TypeID, location.Latitude, location.Longitude, location.LocationName);
        await locationInstance.delete();
      }

      // Delete related records in the Users table
      await connection.query("DELETE FROM Users WHERE BusinessID = ?", [this.BusinessID]);
  
      // Delete the business itself
      await connection.query("DELETE FROM Businesses WHERE BusinessID = ?", [this.BusinessID]);
